...
DW_box<- function(ts){
    ...
    library(ggplot2)
    p <- ggplot(d)+
        geom_boxplot(aes(y=tide,x='Overall'),size=1.2)+   # overall
        # Note: 'Overall' will show as the legend name, can be any char
        geom_boxplot(aes(y = tide,x=D_W))+     # dry, wet only
        scale_y_continuous(breaks=seq(-14,10,2))+
        labs(y="Tide Elev, ft NAVD88",x="Dry/Wet",title="Dry vs. Wet Periods box-whisker plot")+
...
save(p.tide.lf.box,p.tide.hf.box,file="Dry_Wet_tide_box.Rdata")
# export plots
